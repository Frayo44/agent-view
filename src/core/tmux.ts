...
export async function attachWithPty(sessionName: string): Promise<void> {
  return new Promise((resolve) => {
    ...
    // Handle PTY exit (session ended or closed)
    ...
    // Forward stdin to PTY, intercepting Ctrl+Q
    const handleStdin = (data: Buffer) => {
      ...
      // Forward all other input to the PTY
    ...
    function cleanup() {
      ...
      // Clear the screen before returning to TUI
...
export function attachSessionSync(sessionName: string): void {
  ...
  // Unbind Ctrl+Q (restore default behavior)
