...
/**
 * Generate a worktree directory path based on the repository directory,
 * branch name, and location strategy.
 *
 ...
 */
export function generateWorktreePath(repoDir: string, branchName: string, location: string = "subdirectory"): string {
  ...
  if (location.includes("/") || location.startsWith("~")) {
    ...
    return path.join(expanded, repoName, sanitized)
  }
